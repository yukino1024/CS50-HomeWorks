...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        revision = False
        #save the same litter's position in x and y
        x_p , y_p = self.crossword.overlaps[x,y]
        delete = []
        for word1 in self.domains[x]:
            ok = 0
            for word2 in self.domains[y]:
                if word1[x_p] == word2[y_p]:
                    ok += 1
            #in y's words don't have "ok" word
            if ok == 0:
                delete.append(word1)
                revision = True
        for word in delete:
        ...
        while arcs:
            ...
            if self.revise(arc[0],arc[1]):
                if not self.domains[arc[1]]:
                    return False
                for v in self.crossword.neighbors(arc[0]):
                    if v != arc[1]:
                        arcs.append((v,arc[0]))
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        for neighbor in neighbors:
            if neighbor in assignment.keys():
                delete.append(neighbor)
        for d in delete:
            neighbors.remove(d)
        #count each value's "limit"
        for value in self.domains[var]:
            count = 0
            for neighbor in neighbors:
                x_p, y_p = self.crossword.overlaps[var,neighbor]
                char = value[x_p]
